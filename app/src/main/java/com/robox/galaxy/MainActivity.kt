...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Use View Binding (remove duplicate setContentView)
    ...
    private fun fetchEnrolledCourses() {
        ...
        GlobalScope.launch(Dispatchers.Main) {
            try {
                Log.d(
                    TAG,
                    "fetchEnrolledCourses: Querying users collection for email: ${user.email}"
                )
                db.collection("users")
                    ...
                    .addOnSuccessListener { studentResult ->
                        Log.d(
                            TAG,
                            "fetchEnrolledCourses: User query succeeded, result size: ${studentResult.size()}"
                        )
                        if (studentResult.isEmpty) {
                            Log.i(
                                TAG,
                                "fetchEnrolledCourses: No user found for email: ${user.email}"
                            )
                            Toast.makeText(
                                this@MainActivity,
                                "No student found",
                        ...
                        Log.d(
                            TAG,
                            "fetchEnrolledCourses: Fetched enrollment number (Eno): $enrollmentNumber"
                        )
                        if (enrollmentNumber.isEmpty()) {
                            Log.w(
                                TAG,
                                "fetchEnrolledCourses: No Eno found, value: '$enrollmentNumber'"
                            )
                            Toast.makeText(
                                this@MainActivity,
                                "No enrollment number found",
                                Toast.LENGTH_SHORT
                            ).show()
                            return@addOnSuccessListener
                        }
                        ...
                        db.collection("Enrollments")
                            ...
                            .addOnSuccessListener { enrollmentResult ->
                                Log.d(
                                    TAG,
                                    "fetchEnrolledCourses: Enrollment query succeeded, result size: ${enrollmentResult.size()}"
                                )
                                val courseIds = mutableListOf<String>()
                                for (doc in enrollmentResult) {
                                    val courseId =
                                        doc.getString("courseID") // Note: courseID (case-sensitive)
                                    if (courseId != null) {
                                        courseIds.add(courseId)
                                        Log.d(
                                            TAG,
                                            "fetchEnrolledCourses: Found course ID: $courseId"
                                        )
                                    } else {
                                        Log.w(
                                            TAG,
                                            "fetchEnrolledCourses: courseId is null in document: ${doc.id}"
                                        )
                                    }
                                }
                                ...
                                if (courseIds.isEmpty()) {
                                    Log.w(
                                        TAG,
                                        "fetchEnrolledCourses: No course IDs found in enrollments"
                                    )
                                }
                                ...
                                Log.d(
                                    TAG,
                                    "fetchEnrolledCourses: Cleared courses list, new size: ${courses.size}"
                                )
                                for (courseId in courseIds) {
                                    db.collection("Courses")
                                        ...
                                        .addOnSuccessListener { courseDoc ->
                                            Log.d(
                                                TAG,
                                                "fetchEnrolledCourses: Course document fetched for ID: $courseId"
                                            )
                                            val course = courseDoc.toObject(Course::class.java)
                                            Log.d(
                                                TAG,
                                                "fetchEnrolledCourses: Course document fetched for ID: $course"
                                            )
                                            if (course != null) {
                                                ...
                                                Log.d(
                                                    TAG,
                                                    "fetchEnrolledCourses: Added course: $course "
                                                )
                                            } else {
                                                Log.e(
                                                    TAG,
                                                    "fetchEnrolledCourses: Failed to convert course data for ID: $courseId"
                                                )
                                            }
                                        ...
                                        .addOnFailureListener { error ->
                                            Log.e(
                                                TAG,
                                                "fetchEnrolledCourses: Error loading course for ID $courseId: ${error.message}"
                                            )
                                            Toast.makeText(
                                                this@MainActivity,
                                                "Error loading a course: ${error.message}",
                                                Toast.LENGTH_SHORT
                                            ).show()
                                        }
                                }
                                ...
                                Log.d(
                                    TAG,
                                    "fetchEnrolledCourses: Notifying adapter of data change, courses size: ${courses.size}"
                                )
                            ...
                            .addOnFailureListener { error ->
                                Log.e(
                                    TAG,
                                    "fetchEnrolledCourses: Error loading enrollments: ${error.message}"
                                )
                                Toast.makeText(
                                    this@MainActivity,
